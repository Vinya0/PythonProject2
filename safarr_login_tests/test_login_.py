...
def driver_init(request):
    ...
    # Create a WebDriverWait instance and assign it to the class
    wait = WebDriverWait(driver, 15)

    request.cls.driver = driver
    request.cls.wait = wait  # <-- assign wait here
...
class TestSafarrLogin:
    ...
    def invalid_format_error(self):
        return self.wait.until(
            EC.visibility_of_element_located(
                (By.XPATH, f"//div[contains(text(),'{validation_assert.INVALID_EMAIL}')]")
            )
        )
    ...
    def test_valid_email_invalid_password(self):
        ...
        self.password_input_field().send_keys(input_field.INCORRECT_PASSWORD[0])
        self.login_button().click()
        assert self.invalid_credentials_error().text == validation_assert.INCORRECT_CREDENTIALS
    ...
    def test_invalid_email_format(self):
        self.refresh_page_and_wait()
        self.email_input_field().send_keys(input_field.INVALID_EMAIL[4])
        self.password_input_field().send_keys(config.CORRECT_PASSWORD)
        self.login_button().click()
        assert self.invalid_format_error().text == validation_assert.INVALID_EMAIL
